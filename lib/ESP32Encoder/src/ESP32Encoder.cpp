...
int ESP32Encoder::getLastValue()
{
	return lastValue;
}

void ESP32Encoder::setLastValue(int val)
{
	lastValue = val;
}

// MIDI ---------------------------------------------------------------------------------------------
void ESP32Encoder::setMIDIParam(u_char _ccNumber, u_char _midiChannel)
{
	ccNumber = _ccNumber;
	midiChannel = _midiChannel;
}

void ESP32Encoder::setCCNumber(u_char _ccNumber)
{
	ccNumber = _ccNumber;
}
void ESP32Encoder::setMIDIChannel(u_char _midiChannel)
{
	midiChannel = _midiChannel;
}

u_char ESP32Encoder::getControlNumber()
{
	return ccNumber;
}
u_char ESP32Encoder::getCurrentCCMessage()
{
	_ENTER_CRITICAL();
	currentCount = getCount() / 2;
	if (lastValue != currentCount)
	{
		if (currentCount < 0)
		{
			currentCount = 127;
		...
		setLastValue(currentCount);
	}
	_EXIT_CRITICAL();
	return currentCount;
}

u_char ESP32Encoder::getMidiChannel()
{
	return midiChannel;
}
//---------------------------------------------------------------------------------------------------
